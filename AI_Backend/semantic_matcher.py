...
async def get_unified_analysis(candidate_data, jd_requirements, role_name):
    """
...
"""
        ...
        print("Calling OpenAI API (Fast Mode)...")
        ...
        for item in exp_analysis:
            print(
                f"  Job Index {item.get('job_index')}: Relevance -> {item.get('relevance_level')}"
            )
        ...
        for item in responsibility_analysis:
            print(f"  ID {item.get('responsibility_id')}: Match -> {item.get('match_level')}")
