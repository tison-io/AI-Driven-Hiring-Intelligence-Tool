...
export class AiService {
  ...
  async evaluateCandidate(
    ...
    try {
      ...
      if (extractedData.is_valid_resume === false) {
        this.logger.warn(
          `AI marked resume as invalid. Reason: ${extractedData.error || 'Unknown'}`,
        );
      ...
      if (!extractedData.candidate_name && !extractedData.skills?.length) {
        this.logger.warn(
          'Insufficient data extracted, falling back to mock data',
        );
        return this.getMockResponse();
      }
      ...
      const scoringResult = await this.scoreCandidateData(
        extractedData,
        jobRole,
        jobDescription,
      );
  ...
  private async scoreCandidateData(
    candidateData: ExtractedCandidateData,
    jobRole: string,
    customJobDescription?: string,
  ): Promise<ScoringResult> {
    const jobDescription =
      customJobDescription || this.getJobDescription(jobRole);

    const response = await axios.post(
      `${this.aiServiceUrl}/score`,
      {
  ...
  private transformAiResponse(
    ...
    const keyStrengths =
      scoringResult.key_strengths?.map((s) =>
        typeof s === 'string' ? s : s.strength || JSON.stringify(s),
      ) || [];

    const potentialWeaknesses =
      scoringResult.potential_weaknesses?.map((w) =>
    ...
    const relevantExperience =
      breakdown.relevant_years_calculated !== undefined
    ...
    return {
      ...
      workExperience:
        extractedData.work_experience?.map((job) => ({
          company: job.company || '',
  ...
  private getJobDescription(jobRole: string): string {
    ...
    const jobDescriptions = {
      'Backend Engineer':
        'Develop server-side applications using Node.js, Python, or Java. Experience with databases, APIs, and cloud services required.',
      'Frontend Developer':
        'Build user interfaces using React, Vue, or Angular. Strong HTML, CSS, JavaScript skills required.',
      'Full Stack Developer':
        'Work on both frontend and backend development. Experience with modern web frameworks and databases.',
      'Data Analyst':
        'Analyze data using SQL, Python, R. Experience with data visualization tools and statistical analysis.',
      'DevOps Engineer':
        'Manage CI/CD pipelines, cloud infrastructure, and deployment automation. Docker, Kubernetes experience preferred.',
    };

    return (
      jobDescriptions[jobRole] ||
      `Professional role requiring relevant technical skills and experience in ${jobRole}.`
    );
  }
  ...
  async extractSkills(rawText: string): Promise<string[]> {
    try {
      ...
      return ['JavaScript', 'Python', 'React'].filter(
        () => Math.random() > 0.7,
      );
    }
  }
