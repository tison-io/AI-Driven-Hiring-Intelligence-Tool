  ...
  private async makeRapidApiRequest(
    ...
    for (let attempt = 1; attempt <= this.apifyConfig.maxRetries; attempt++) {
      try {
        ...
        this.logger.debug(
          `Using API key: ${this.apifyConfig.rapidApiKey?.substring(0, 10)}...`,
        );
        this.logger.debug(
          `Endpoint: ${this.apifyConfig.linkedinScraperEndpoint}`,
        );
