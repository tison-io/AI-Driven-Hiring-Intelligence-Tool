...
import {
  LinkedInPublicDataDto,
  PublicExperienceDto,
  PublicEducationDto,
  PublicSkillDto,
} from '../dto/linkedin-public-data.dto';
import {
  RapidApiProfileData,
  RawExperience,
  RawEducation,
  RawSkill,
} from '../interfaces/linkedin-raw-api.interface';

@Injectable()
export class LinkedInMapper {
  private readonly logger = new Logger(LinkedInMapper.name);

  async transformToPublicData(
    profileData: RapidApiProfileData,
  ): Promise<LinkedInPublicDataDto | null> {
    try {
      ...
      const publicData = {
        fullName,
        headline:
          profileData.basic_info?.headline || profileData.headline || '',
        location:
          profileData.basic_info?.location?.full || profileData.location || '',
        experiences: this.mapExperiences(
          profileData.experience || profileData.experiences || [],
        ),
        educations: this.mapEducations(
          profileData.education || profileData.educations || [],
        ),
        skills: this.mapSkills(
          profileData.basic_info?.top_skills || profileData.skills || [],
        ),
      };
  ...
  async transformMultipleProfiles(
    profiles: RapidApiProfileData[],
  ): Promise<LinkedInPublicDataDto[]> {
  ...
  private mapEducations(educations: RawEducation[]): PublicEducationDto[] {
    return educations
      ...
      .map((edu) => ({
        ...
        startDate: edu.start_date?.year
          ? edu.start_date.year.toString()
          : edu.startDate || undefined,
        endDate: edu.end_date?.year
          ? edu.end_date.year.toString()
