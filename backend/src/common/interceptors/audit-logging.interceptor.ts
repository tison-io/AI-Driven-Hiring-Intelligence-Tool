...
export class AuditLoggingInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (
      request.url.includes('/admin/audit-logs') ||
  ...
  private getActionName(
    ...
    if (url.includes('/register') && httpMethod === 'POST')
    ...
    if (url.includes('/profile') && httpMethod === 'GET')
      return 'USER_PROFILE_ACCESSED';
    ...
    if (
      ...
      httpMethod === 'GET' &&
      url.match(/\/candidates\/[^/]+$/)
    )
      return 'CANDIDATE_PROFILE_ACCESSED';
    if (url.includes('/candidates') && httpMethod === 'POST')
      return 'CANDIDATE_RECORD_CREATED';
    if (url.includes('/candidates') && httpMethod === 'PUT')
      return 'CANDIDATE_RECORD_UPDATED';
    if (
      url.includes('/candidates') &&
      httpMethod === 'PATCH' &&
      url.includes('/shortlist')
    )
      return 'CANDIDATE_SHORTLIST_TOGGLED';
    if (url.includes('/candidates') && httpMethod === 'DELETE')
      return 'CANDIDATE_RECORD_DELETED';
    ...
    if (url.includes('/dashboard/admin') && httpMethod === 'GET')
      return 'ADMIN_DASHBOARD_DATA_ACCESSED';
    if (url.includes('/dashboard/score-distribution') && httpMethod === 'GET')
      return 'CANDIDATE_SCORE_ANALYTICS_ACCESSED';
    ...
    if (url.includes('/privacy/retention-policy'))
      return 'DATA_RETENTION_POLICY_ACCESSED';
  ...
  private getDetails(httpMethod: string, url: string, request: any): string {
    ...
    if (url.includes('/logout'))
      return 'User session terminated and logged out';
    if (url.includes('/change-password'))
      return 'User password updated successfully';
    if (url.includes('/complete-profile'))
      return 'User profile information updated with new data';
    if (url.includes('/forgot-password'))
      return 'Password reset request initiated';
    if (url.includes('/reset-password'))
      return 'Password successfully reset using secure token';
    if (url.includes('/profile') && httpMethod === 'GET')
      return 'User profile data retrieved';

    // Candidate operations
    if (url.includes('/upload-resume'))
      return 'Resume file uploaded and processed for evaluation';
    if (url.includes('/linkedin'))
      return 'LinkedIn profile data extracted and processed';
    if (
      url.includes('/candidates') &&
      httpMethod === 'GET' &&
      url.includes('/candidates/')
    )
      return 'Detailed candidate information accessed';
    if (url.includes('/candidates') && httpMethod === 'GET')
      return 'Candidates list accessed with filters';
    if (url.includes('/candidates') && httpMethod === 'POST')
      return 'New candidate record created';
    if (url.includes('/candidates') && httpMethod === 'PUT')
      return 'Candidate information updated';
    if (url.includes('/shortlist')) return 'Candidate shortlist status toggled';
    if (url.includes('/candidates') && httpMethod === 'DELETE')
      return 'Candidate record and PII data permanently deleted';
    ...
    if (url.includes('/export/report'))
      return 'Detailed candidate report generated';

    // Dashboard operations
    if (url.includes('/dashboard/admin'))
      return 'Admin dashboard with system metrics accessed';
    if (url.includes('/dashboard/score-distribution'))
      return 'Score distribution analytics accessed';
    if (url.includes('/dashboard'))
      return 'Dashboard analytics and metrics accessed';

    // Privacy operations
    if (url.includes('/privacy/export-data'))
      return 'User data exported for GDPR compliance';
    if (url.includes('/privacy/delete-data'))
      return 'User data permanently deleted (Right to be forgotten)';
    if (url.includes('/privacy/policy'))
      return 'Privacy policy document accessed';
    if (url.includes('/privacy/retention-policy'))
      return 'Data retention policy accessed';
